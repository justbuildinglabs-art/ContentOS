...
ContentOS is a Claude Code plugin: a four-stage Instagram Reels content pipeline (research → direct → write → qa). The design spec is `docs/superpowers/specs/2026-09-16-contentos-design.md` and is the binding authority.
...
- Standard library only. No pip dependencies. ffmpeg is optional at runtime (keyframes) and only touched by `lib/frames.py`.
...
- Founder state lives under `<project>/.contentos/`, never in this repo.
...
- Founder-facing text: plain language, short sentences, no em dashes.
