...
- The printed table opens with the floors in force, lists Established then Rising, then one "Left out" line with counts per reason and the breakout count, then `RESULT {...}`. `run_discover` returns the data and reports progress through `log`; printing lives in the CLI handler so the panel can reuse it.
