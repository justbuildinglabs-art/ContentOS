...
- Pass 1 (details, steps A and B): `not_found`, `error`, `private`; "under 10,000 followers" (the floor in force); "no reel in 30 days" when `latestPosts` is present and none of its unpinned reels (`productType: "clips"`) is 30 days old or newer. `latestPosts` holds up to 12 posts, up to 3 pinned ones first. Its reels carry `videoViewCount`, never `videoPlayCount`, so pass 1 never judges views.
