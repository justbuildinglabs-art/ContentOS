...
- `discover --hashtags a,b,c [--keywords "x,y"] [--handles-file <path>] [--yes] [--estimate-only]` makes up to three runs of the same `apify~instagram-scraper` actor. Run A: `directUrls` of `https://www.instagram.com/explore/tags/<tag>/`, `resultsType: "reels"`, `resultsLimit` `discover_reels_per_hashtag` (default 30, an integer 1 or more), `onlyPostsNewerThan` `<lookback_days> days`. Run B, only with keywords: `search`, `searchType: "profile"`, `searchLimit` 10, `resultsType: "details"`. Both input shapes are UNVERIFIED until the 0.5.0 probe run pins them.
...
- Instagram draws its "Paid partnership" label over the player. It is not in the video file, so no keyframe can show it. The check therefore reads what the pipeline does have.
- `sponsored.detect(reel, transcript_text=None)` returns `{"detected": bool, "signals": [...]}`. Signals: a sponsored flag on the raw scraper item when the probe run finds one; a hashtag in `ad`, `sponsored`, `sponsoredpost`, `paidpartnership`, `paidpartner`, `brandpartner`, `gifted`; and a caption or transcript phrase in "paid partnership", "sponsored by", "in partnership with", "partnered with", "thanks to ... for sponsoring". Matching is case-insensitive on whole hashtags and word boundaries, so `#adventure`, `#advice`, and "partner workout" never match.
