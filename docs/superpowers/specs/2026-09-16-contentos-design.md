...
- `ui [--mock] [--port N] [--open] [--idle-minutes 60]` serves one page on 127.0.0.1 (the OS picks the port unless `--port`), writes `.contentos/ui-session.json` (`url, pid, started_at`), prints the URL, and blocks until the person saves or closes, or until `--idle-minutes` pass with no request. It then removes the session file, prints `RESULT {"saved", "picks", "settings", "discovery_path"}`, and exits 0. The skill starts it in the background, opens the URL (its browser tool when present, else `--open`), and continues when the process exits. Python stays standard library (`http.server`, `threading`, `secrets`).
...
- After a run: progress lines, then Established and Rising rows with ticks and the niche evidence (a warning when no measured reel matches a niche term), the left-out counts with reasons, and the breakout count. Save merges the picks into `competitors` after the current ones through `setup.run_accounts` (format accounts untouched). Before setup there is no `config.json`, so Save writes `.contentos/discovery-picks.json` (`{"picks": [...]}`) and setup round 4 reads it. "Remember these settings" writes the dials into `config.json` through `store.update_config_keys` (validated like `load_config`), only once setup has run.
- Routes: `GET /`, `GET /api/state`, `POST /api/estimate`, `POST /api/run`, `GET /api/status`, `GET /api/discovery`, `POST /api/save`, `POST /api/close`. `App.handle(method, path, headers, body)` is pure and tested without sockets; a thin `ThreadingHTTPServer` adapter serves it. One run at a time, on a worker thread.
- Safety: every API call needs the session token (random, from `secrets`, in the URL the skill opens, sent back as a header); the Host header must be `127.0.0.1:<port>` or `localhost:<port>`; the key never reaches the page; POST bodies must be JSON. The page is one file with inline CSS and plain JS, loads nothing from outside, and puts scraped text (bios, captions) into the page with `textContent` only.
