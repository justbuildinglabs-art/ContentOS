...
- Shortlist: pass-1 survivors that are not seeds, niche hits first (a niche term in the bio or in a `latestPosts` caption or hashtag), then Established and Rising alternating by followers. The first `discover_shortlist` (default 20, an integer 1 or more) go to step C and the rest are dropped as "shortlist full".
