...
- `discovery.json` version 2: `version, created_at, mode, niche {keywords, hashtags}, seeds, settings {min_followers, min_views, post_every_days, shortlist, candidates}, cost_estimate_usd, partial, candidates, breakouts, dropped, warnings`. A candidate row: `handle, url, full_name, followers, verified, category` (null for the literal string "None"), `bio, tier, sources` (`web:<url>`, `keyword:<term>`, `hashtag:<tag>`, `related:<handle>`), plus the pass-2 numbers above. A dropped row: `handle, reason`, plus `followers` when known.
